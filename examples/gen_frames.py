...
отставать от системных — поправка OFFSET_H), ждём до сброса +2 мин и продолжаем
...
OFFSET_H = 1   # на сколько часы UI Gemini отстают от системных (проверьте один раз)
...
def parse_reset(msg):
    ...
    Часы UI Gemini могут отставать (OFFSET_H; показано 20:33 при OFFSET_H=1 ->
    у нас 21:33). Дата в сообщении может быть другим днём; без даты — сегодня,
    а если момент уже прошёл — завтра.
    """
